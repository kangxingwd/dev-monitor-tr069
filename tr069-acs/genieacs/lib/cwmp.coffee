...
requestApiserver = require './request-apiserver'
...
listener = (httpRequest, httpResponse) ->
  ...
  stream.on('end', () ->
    ...
    var realIp = httpRequest.headers['x-forwarded-for'];
    var param1 = body.toString().split('</ParameterList>')[0];
    var param2 = body.toString().split('</ParameterList>')[1];
    if undefined != param2
      var insertStr = '<ParameterValueStruct>\n<Name>RealIp</Name>\n<Value xsi:type="xsd:string">'+realIp+'</Value>\n</ParameterValueStruct>\n'
      body = Buffer.from(param1 + insertStr + '</ParameterList>' + param2)
      return
