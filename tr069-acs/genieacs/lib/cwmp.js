...
transferComplete = function(sessionContext, rpc) {
  return session.transferComplete(sessionContext, rpc.cpeRequest, function(err, acsResponse, operation, fault) {
    ...
    // kx add
    var current_channel = {};
    if( operation != null && operation.hasOwnProperty("channels")){
      for (channel in operation.channels) {
          if(operation.channels.hasOwnProperty(channel)){
    ...
    if (fault) {
      ...
      recordFault(sessionContext, fault, operation.provisions, operation.channels);
        // kx add TransferComplete
        //console.log("------------ transferComplete fault-----------");
        var requestParams_1 = {
            ...
            data: {
                ...
                fault:    fault,
                retries:  sessionContext.retries[current_channel]?sessionContext.retries[current_channel]:0
            }
        };
        requestApiserver.postFormJson(requestApiserver.eventUrl.transferComplete, requestParams_1,  function(result) {
           // console.log("------------ request cwmp_event transferComplete fault-----------");
           // console.log(result);
           // console.log("------------------------------------------\n\n");
        });
    }else {
        // kx add TransferComplete
        //console.log("------------ transferComplete success-----------");
        var requestParams_2 = {
            event: 'transferComplete',
            data: {
                channel:  current_channel,
        ...
        requestApiserver.postFormJson(requestApiserver.eventUrl.transferComplete, requestParams_2,  function(result) {
           // console.log("------------ request cwmp_event transferComplete success-----------");
           // console.log(result);
           // console.log("------------------------------------------\n\n");
        });
    }
